...
public class Startup
{
    ...
    {
        services.AddDbContextFactory<EquipmentDbContext>(options => {
            options.UseSqlServer(Configuration.GetConnectionString("LOT5RDB"));
        });
        services.AddDbContextFactory<DescriptionsDbContext>(options => {

            options.UseCosmos(Configuration.GetConnectionString("LongTextDb"), "Descriptions");
        });
